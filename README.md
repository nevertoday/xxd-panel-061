...
一张旅行、聚会或日常照片里，真正舍不得删掉的往往不只一个主体：一个动作、一件小物、一处环境和人物之间的关系，都可能是记忆的一部分。普通的单主体插画会丢掉这些线索，平均分格的拼贴又容易像素材清单。

**Panel 061** 把最有意义的 3–6 个线索重组为彼此呼应的记忆片段。它保留照片的核心关系，又用轻盈手作质感和大面积留白建立清楚的叙事路径，让画面像一页经过编辑的私人视觉日记。

### 适合这些情况

- **一张照片里有多个舍不得丢的细节：** 人物、动作、物件与环境线索需要共同讲述同一段记忆。
- **想做拼贴但不想得到素材陈列：** 片段需要有主次、方向和呼应，而不是平均分格或随机散落。
- **偏爱温柔手作感又怕显得幼稚：** 剪纸、水粉、粉蜡和 Riso 质感由专业留白与编辑秩序托住。
...
> 用 XXD Panel 061 帮我处理这张图，先推荐最合适的构图和尺寸。
...
## 快速判断：XXD Panel 061 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 把照片重组为散落而彼此呼应的视觉记忆片段 |
| **一眼可辨的特点** | 选择性记忆 · 多片段叙事 · 剪纸色块 · 治愈色盘 · 即兴编辑排版 |
...
先识别照片里真正值得记住的核心主题，再挑出约 3–6 个主体、动作、环境线索或隐喻，把它们变成彼此呼应的视觉记忆片段。片段不是随机摆放，而是在大面积留白中形成一条可阅读的私人叙事路径。

```text
识别核心主题与叙事关系 → 选择 3–6 个记忆片段 → 用剪纸、水粉、粉蜡与 Riso 语言概括 → 建立大小、方向与疏密呼应 → 从原图转译治愈色盘 → 让少量文字成为记忆线索
```

## 成品中最容易识别的风格特点

- 约 3–6 个片段围绕同一个主题共同叙事，不做随机物件陈列。
- 剪纸色块、水粉、粉蜡、Risograph 与丝网印刷质感共同形成轻盈手作语言。
- 构图看似随手，却通过尺度反差、斜向呼应、正负形和疏密建立稳定阅读路径。
- 背景接近白色并随原图冷暖轻微变化；大面积留白让片段保持距离和呼吸。
- 文字像散落的观察笔记或短句，与图形发生连接，而不是形成菜单式信息栏。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-061-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 061 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-061 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-061 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-061 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 061；公共设置只解析一次
/xxd-panel-061 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-061`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-061-prompt.en.md) · [中文运行适配器](references/xxd-panel-061-prompt.zh-CN.md)
...
| **[xxd-panel-061](https://github.com/nevertoday/xxd-panel-061)** | 黑色主形 · 极大留白 · 网点消散 · 禅意思考 · 哲思碎片 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–061） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |

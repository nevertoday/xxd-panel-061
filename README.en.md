...
That frustration is why I made **Panel 061**. It does not press a template onto your image. It first works out what must not be lost, then translates it into **Reorganize photos into scattered and echoing visual memory fragments**.
...
**Keywords:** Selective memory · Multi-fragment narrative · Paper-cut color blocks · Healing color palette · Improvised editing and layout

Panel 061 selects about 3-6 of the most meaningful subjects, actions, environmental clues or metaphors from the photos, and uses paper cutting, gouache, powder wax, Risograph and screen printing graphic language to organize them into a page of light personal visual diary.

<!-- xxd-panel-benefit:start -->
## Quick fit check: is XXD Panel 061 right for you?

| What you need to know | What this style gives you |
|---|---|
| **What you will get** | Reorganize photos into scattered and echoing visual memory fragments |
| **Recognizable signature** | Selective memory · Multi-fragment narrative · Paper-cut color blocks · Healing color palette · Improvised editing and layout |
...
It first identifies the source's central memory, then selects roughly three to six subjects, actions, environmental clues, or metaphors and turns them into related visual-memory fragments. The pieces are not scattered at random; they form a readable private narrative across generous negative space.

```text
identify the core theme and narrative relation → select three to six memory fragments → simplify through cut paper, gouache, wax pastel, and Riso language → build echoes through scale, direction, and spacing → translate a healing palette from the source → let sparse copy act as a memory clue
```

## What makes the finished style recognizable

- Roughly three to six fragments tell one shared story instead of becoming a random object inventory.
- Cut-paper blocks, gouache, wax pastel, Risograph, and screen-print texture create a light handmade language.
- The layout feels spontaneous but uses scale contrast, diagonal echoes, positive-negative shapes, and density to form a stable reading path.
- The ground stays near-white and shifts gently with the source temperature; generous space keeps every fragment breathing.
- Copy behaves like scattered observation notes or short phrases that connect the imagery rather than forming an information menu.

Aesthetic constraints and rejection rules live only in the [original source brief](references/original-prompt/zh-CN.md); the Skill and runtime adapter handle delivery variables only. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-061-prompt.en.md)
...
/xxd-panel-061 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB
...
/xxd-panel-061 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP
...
/xxd-panel-061 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-061 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-061 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 061 to a whole directory; shared settings resolve once
/xxd-panel-061 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-061`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-061-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-061-prompt.zh-CN.md)
...
| **[xxd-panel-061](https://github.com/nevertoday/xxd-panel-061)** | BLACK PRIMARY FORM · IMMENSE NEGATIVE SPACE · HALFTONE DISSOLUTION · ZEN REFLECTION · FRAGMENTS OF THOUGHT |
...
| **Soldiers** | `xxd-panel-NNN` (currently 001–061) | Each numbered Skill executes only its own original brief and aesthetic, completing the individual job assigned by the General. |

...
旅先や集まり、日常の一枚で、本当に残したいものは一つの主役だけとは限りません。何気ない動作、小さな持ち物、その場の風景、人物同士の関係まで、すべてが記憶の一部になり得ます。一般的な単一主体のイラストではこうした手掛かりが失われ、均等に分割したコラージュでは素材一覧のように見えがちです。

**Panel 061** は、最も意味のある 3〜6 個の手掛かりを、互いに響き合う記憶の断片として再構成します。写真の核となる関係を残しながら、軽やかな手仕事の質感と大きな余白で明確な物語の流れをつくり、丁寧に編集された個人的なビジュアル日記の一ページへ仕上げます。

### このような場面に適しています

- **一枚の写真に残したい細部がいくつもある：** 人物、動作、物、周囲の手掛かりを一緒に使い、同じ記憶を語らせたい。
- **コラージュにしたいが、素材の陳列にはしたくない：** 均等な分割や無作為な配置ではなく、断片に強弱、方向、呼応を持たせたい。
- **やさしい手仕事感が好きだが、幼く見えるのは避けたい：** 切り紙、ガッシュ、パステル、Riso の質感を、洗練された余白と編集的な秩序で支えたい。
- **柔軟な納品形式が必要：** 上下・左右比較、デザインのみ、複数比率、4 デバイス向け壁紙に対応し、画像フォルダーも一括処理したい。

### 簡単な始め方

最初からすべての設定を読む必要はありません。画像を Agent に渡し、次のように伝えてください。

> XXD Panel 061 でこの画像を処理して、まず最適な構図とサイズを提案して。

上の作例で視覚方向を確認できます。準備ができたら[使い始める](#使い始める)へ進めます。全モード、サイズ、引数は後半の折りたたみ資料から必要に応じて確認できます。
<!-- xxd-human-intro:end -->
...
**キーワード:** 選択的記憶 · 複数断片の物語 · 切り絵のカラーブロック · 癒しのカラーパレット · 即興編集とレイアウト

Panel 061 は、写真から最も意味のある主題、行動、環境に関するヒントや比喩を約 3 ～ 6 つ選択し、切り紙、ガッシュ、パウダーワックス、リソグラフ、スクリーン印刷のグラフィック言語を使用して、それらを軽い個人的なビジュアル日記のページに整理します。

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 061 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 写真を散在してエコーする視覚的記憶の断片に再編成する |
| **ひと目で分かる特徴** | 選択的記憶 · 複数断片の物語 · 切り絵のカラーブロック · 癒しのカラーパレット · 即興編集とレイアウト |
...
まず情報源の中心記憶を特定し、次におよそ 3 ～ 6 つの主題、行動、環境の手がかり、または比喩を選択し、それらを関連する視覚記憶の断片に変換します。断片はランダムに散らばっているわけではありません。それらは、広々としたネガティブスペースに読みやすいプライベートな物語を形成します。

```text
核となるテーマと物語の関係を特定する → 3 ～ 6 つの記憶の断片を選択する → カット紙、ガッシュ、ワックスパステル、リソ言語を通じて単純化する → スケール、方向、間隔を通じてエコーを構築する → ソースから癒しのパレットを翻訳する → まばらなコピーを記憶の手掛かりとして機能させる
```

## 完成作品を見分けるポイント

- およそ 3 ～ 6 個の断片が、ランダムなオブジェクトの目録になるのではなく、1 つの共有ストーリーを伝えます。
- カットペーパーブロック、ガッシュ、ワックスパステル、リソグラフ、スクリーンプリントのテクスチャーが、軽い手作りの言語を生み出します。
- レイアウトは自発的に感じられますが、スケール コントラスト、斜めのエコー、ポジ/ネガの形状、密度を使用して、安定した読み取りパスを形成します。
- 地面はほぼ白のままで、ソースの温度に応じて緩やかに変化します。寛大なスペースにより、すべての断片が呼吸し続けます。
- コピーは、情報メニューを形成するのではなく、散在する観察メモや画像を接続する短いフレーズのように動作します。

美的制約と避けるべき表現は[中国語の原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは今回の納品条件だけを扱います。 [Skill ワークフロー](SKILL.md) · [英語ランタイムアダプター](references/xxd-panel-061-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 061 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-061 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-061 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-061 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-061 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 061 を適用し、共通設定は一度だけ解決
/xxd-panel-061 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-061`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-061-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-061-prompt.zh-CN.md)
...
| **[xxd-panel-061](https://github.com/nevertoday/xxd-panel-061)** | 黒い主形 · 巨大な余白 · 網点の消散 · 禅的思考 · 思考の断片 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–061） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
